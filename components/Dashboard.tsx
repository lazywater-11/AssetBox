...
const Dashboard: React.FC<DashboardProps> = ({ state, exchangeRate, onNavigate }) => {
  ...
  const getChartData = (period: Period) => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      if (val !== undefined) {
        const label = days <= 30 ? dateStr.slice(5) : dateStr.slice(0, 7);
        data.push({ name: label, value: val, fullDate: dateStr });
      }
    }
    return data;
  };
      ...
      <div className="bg-brand-card rounded-2xl p-6 shadow-2xl shadow-black/30 border border-white/5">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex bg-black/30 rounded-lg p-1">
            {(['1M', '3M', '1Y', '3Y', '5Y'] as Period[]).map(t => (
              <button
                key={t}
                onClick={() => setActivePeriod(t)}
                className={`px-3 py-1 text-xs rounded-md transition-colors ${activePeriod === t ? 'bg-brand-green text-black font-bold' : 'text-brand-muted hover:text-white'
                  }`}
              >
                {t}
              </button>
